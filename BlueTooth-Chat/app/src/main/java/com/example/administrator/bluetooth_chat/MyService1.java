...
public class MyService1 {
    ...
    // 构造器
    public MyService1(Chatting activity) {
    ...
    private class ConnectThread extends Thread {
        ...
        public void run() {
            ...
            try {// 连接到BluetoothSocket
                ...
                //连接失败
                activity.myHandler2.sendEmptyMessage(2);
                return;
            }
            synchronized (MyService1.this) {// 将ConnectThread线程置空
    ...
    public void secretWrite(String msg) {//向ConnectedThread写入数据的方法
        ConnectedThread tmpCt;// 创建临时对象引用
        synchronized (this) {// 锁定ConnectedThread
            if (myState != STATE_CONNECTED) return;
            tmpCt = myConnectedThread;
        }
            tmpCt.write(msg + "|secret");// 写入数据
        ...
        public void run() {
            ...
            while (true) {// 一直监听输入流
                try {
                    ...
                    //加密模式
                    if (msgRev.contains("|secret"))
                    {
                        if ( msgRev.contains("|f1") )
                        {
                             if (msgRev.contains("|f2")){
                                 Log.e("text",  msgRev);
                                 msgRev = msgRev.replace(msgRev.substring(msgRev.indexOf("|secret|f1|f2"), msgRev.length()), "");

                                 Message msg =  activity.myHandler2.obtainMessage(1, msgRev);
                                 msg.sendToTarget();
                             }
                             else {
                                 Log.e("text",  msgRev);
